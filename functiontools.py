...
def searchfilter():
    for line in file(output + "content.txt"):
        ...
        for ind,row in d_source.iterrows():
            ...
            if str(row['msginfo']).startswith("#2012伦敦奥运"):
...
def calcwordnum(sent):
    ...
    if '[' not in sent: 
        ...
        if wordlen<80 and wordlen>50:print sent,wordlen
    return wordlen
...
def gettraindata():
    ...
    for line in file(filepath2 + "content.txt"):
        ...
        findactivity = lambda row : str(row["msginfo"]).startswith("#") and r'//@' not in row['msginfo'] and not row['msginfo'].startswith('【')  and "此微博已被删除" not in row['msginfo'] and "分享图片" not in row['msginfo'] and "@" not in row['msginfo'] and "http" not in row['msginfo']
